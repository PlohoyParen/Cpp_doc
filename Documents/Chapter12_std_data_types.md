...
std::cout << a; 	//и прямой вывод строки
//выведет "Hello, world!";
if (n > m)		//поддерживает прямое сравнение 
	//TODO
```
### Методы
- `my_str.find(other_str)` - смотрит, есть ли в my_str подстрока other_str. Возвращает bool тип.
- `my_str.length()` - возвращает длинну строки
- `my_str.to_string(42);` - преобразует число в строку
- `std::stoi(str)` - преобразует строку в число. Например, `int num = std::stoi("69")`.
