...
### Функторы
**Функтор (fucntor)** (или ещё «функциональных объектов») — это классы, которые работают как функции. В основном перегрузка оператора () делается для создания функторов. Например,
```cpp
class Accumulator
{
private:
...
### Где использовать
- Функторы, в первую очередь.
- Двумерные массивы, если очень надо.
