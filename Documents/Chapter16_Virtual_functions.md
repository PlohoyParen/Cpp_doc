...
Если программа предсматривает *полиморфное* повидение некоторого класса, то его деструктор должен быть виртулальным. Суть виртуального деструктора аналогична виртуальным ф-циям: вызов самого дочернего деструктора. Пример, где это необходимо,
```cpp
class Parent
{
public:
