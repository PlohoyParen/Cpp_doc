...
{
public:
	//ничего тут не буду добавлять и переопределять
}; 
```
Более того, если у класса есть *хотя бы одна* абстрактная ф-ция, то это **абстрактный класс**. Это значит, что нельзя создавать объекты этого класса. Он нужет только для наследования и написания полиморфного кода (см. далее про интерфейсы). Так, класс Parent - это абстрактный класс, и, если мы попытаемся создать объект класса Parent, то коспилятор выдаст ошибку.
