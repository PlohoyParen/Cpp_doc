...
### none-type параметр
Параметр **non-type** в шаблоне — это специальный параметр шаблона, который заменяется конкретным значением, а не типом данных. Выглядит это так: `template <class T, int size>`. Non-type параметром может быть: 1) целочисленное значение или перечисление; 2) указатель или ссылка на объект класса; 3) указатель или ссылка на функцию; 4) указатель или ссылка на метод класса; 5) std::nullptr_t. Например, тут none-type параметр используется для задания размера массива:
```cpp
...
```
При этом, этот *параметр должен быть const at-compile-time* (иначе говоря, хранится в static памяти). 
