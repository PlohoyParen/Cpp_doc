...
## Шаблоны ф-ций 
Суть шаблона заключается в том, что компилятор сам подставляет тип данных в ф-цию на основе передаваемого типа данных. **По сути это макрос, для которого в C++ сделали свой синтаксис**. Например, 
```cpp
