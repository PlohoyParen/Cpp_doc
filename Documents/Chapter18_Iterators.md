...
```
Однако, в случаии std::forward_list мы можем только инкрементировать (`++i`). Если же мы хотим передвинуть итератор на 4 вперед, то надо 4 раза инкрементировать. Для этого и нужна `std::advance()`: 
