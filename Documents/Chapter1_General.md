...
3. *(6-8 ячейки)* Выделяется место для возвращаемого значения (ret val): сюда будет сохранен результат возврощаемый ф-цией. Место для адреса возврата (ret addr): т.е. адреса куда вернется выполнение программы после завершения данной ф-ции. Место для какой-то мутной технической инфы про процессор (registers).
...
- **Pointers** Есть 3 указателя (на картинке 2): 1) *Frame pointer* - определяет границы локальный видимости данной ф-ции. Те в его пределах сохраняются только локаные переменные этой ф-ции. 2) *Stack pointer* - указывает на нижнюю границу стэка, ниже память пуста. 3) *Indication pointer* (или как-то так) - указывает на участок памяти (те какой-то объект), обрабатываемый в данный момент.       
##### frame pointer
...
```
Таким образом при входе в B() `frame_pointer` устанавливается на начало ф-ции B(). При этом вся ф-ция A() лежит ниже по стэку. Теперь, выдется память на int a: 4 байта, те 4 ячейки память (тк каждая из них - это 1 байт). Те переменная a сохраняется по адресу: `frame_pointer + 4`. Далее еще 4 байта выделсяется для int b. Адрес переменной b: `frame_pointer + 4 + 4`. Те **отсчет адреса внутри блока идет от frame pointer, указывающего на начало блока**.
...
**Агрегация**(Aggregation) - отношения, где один отдельный класс зависим от другого по типу "Имеет". Например, есть класс Адрес и класс Дом. Дом может использовать Адрес и его методы для своих нужд, но не наооброт.
