...
- `getline(char_type* s, int c_size)` or `getline(char_type* s, int c_size, char delim)` - считывает всю строку до переноса или символа указаного в параметре char delim и сохраняет в строковый объект s (c++ or c-style строку). 
- `read( char_type* s, int n)` - считывает n символов и сохраняет их в строковый объект s (c++ or c-style строку). 
...
- `write( char_type* s, int n)` - зписывает первые n символов строкового объекта s (c++ or c-style строку). Симметрично read().
