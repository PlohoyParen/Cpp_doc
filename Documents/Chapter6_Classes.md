...
- По умолчанию, компилятор будет обрабатывать любой вызов конструктора используя неявное преобразование, если аргументы конструктора в точности не совпали. Преобразование будет сделано неявно, если это родиственные классы и не происходить потеря данных (те "сужения" типа данных, например, float/double -> int). Так преобразование int->float или char->int будет сделано неявно.         
...
/* Копируюшая инициализация, вызовет копирующий конструктор */
