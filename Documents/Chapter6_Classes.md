...
**Конструктор копирования** - это спец вид конструктора, который вызывается при инициализации экземпляра следующим образом: `cpp IntArray a3 = a1; \\ a1 - тоже экземпляр класса IntArray`.    
Пусть у нас есть некоторый класс для хранения массивов (хранит массив данных, его размер и всякие методы). Пусть память в нем выделяется динамически. 
```cpp
class IntArray {
...
private :
