...
**Конструктор копирования** - это спец вид конструктора, который вызывается при инициализации экземпляра следующим образом:
```cpp
IntArray a3 = a1; \\ a1 - тоже экземпляр класса IntArray
```   
Если он не определен специально, то компилятор генерирует его по дефолту. В этом случаи он просто будет копировать все поля и создаст идентичный экземпляр. Это включает идентичные указатели, адреса и динамическую память, отсюда могут быть проблемы в некоторых классах.   
...
```
В этом случаии нужно сделать конструктор копирования, который скоприрует данные и позаботиться о динамической памяти. Например,
```cpp
IntArray(IntArray const & a)
...
```
#### Неявное преобразование к конструктору копирования (конструкторы от одного параметра)
...
При копировнии одного экзепляра такого класса в другой произойдет утечка памяти, так как мы потеряем указатель на одну из их участков динмической памяти (оба указателя будут указывать на второй участок). Решить это можно перегрузкой оператора присваивания. Например,
```cpp
