...
```
Хотя это работает для простых переменных, это не лучший способ это делать тк тут все объекты объявляются, а потом идет присвоение. Грамотно это делать через **список инициализации**, тогда это будет инициализация всех объектов. В нем, после оператора (`:`) идет инициализация членов класса (не обязательно всех):
```cpp
