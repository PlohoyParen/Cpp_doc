...
```cpp
// Есть класс Snake. Его метод Eat принимает экземпляр класса Apple
// Тк Snake не знает про Apple, нам надо сказать компилятору, что это класса. Можно просто добавть слов class в аргументе
