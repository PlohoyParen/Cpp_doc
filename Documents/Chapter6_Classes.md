...
#### Что такое ->
Это синтаксис обозначающий обращение к полю объекта, если объект представлен указателем. Те `a->b` означает: если указатель на некоторый экзепляр класса (`ptr_a = &a`) и у этого класса поле (`b`). Тогда, чтобы обратиться к полю `b` экзепляра `a`, на который указывает `ptr_a`, надо его разыменовать, а потом обратиться к полю через опретор точка (`.`): `(*ptr_a).b`. Для этого придумали короткую запись: `a->b`.    

#### *this 

Любой метод класса работает аналогично обычным ф-циям. Главное отличие это то, что компилятор неявно (скрыто) подставляет туда указать на объект, который вызвал этот метод. Например, ниже идет обычный простой класса с геттером и сеттером.
```cpp
...
public:
    ...
    int getNumber(Another* const this) { return this->m_number; }		//обычный геттер
};
