...
```   
Если он не определен специально, то по компилятор генерирует его по дефолту. В этом случаи он просто будет копировать все поля и создаст идентичный экщемпляр. Это включает идентичные указатели, адреса и динамическую память, отсюда могут быть проблемы в некоторых классах.   
